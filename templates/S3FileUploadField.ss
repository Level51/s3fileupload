			...
			<li class="ss-uploadfield-item template-download" data-fileid="$ID">
				...
				<div class="ss-uploadfield-item-info">
					...
					<label class="ss-uploadfield-item-name">
						...
						<span class="size">$SizeForHuman</span>
						<div class="clear"><!-- --></div>
					</label>
		...
		<div class="ss-uploadfield-item-info">
			...
			<% if $canUpload %>
				<label class="ss-uploadfield-fromcomputer ss-ui-button ui-corner-all" title="<% _t('UploadField.FROMCOMPUTERINFO', 'Upload from your computer') %>" data-icon="drive-upload">
					...
					<input id="$id" name="file" class="$extraClass ss-uploadfield-fromcomputer-fileinput" data-config="$configString" type="file"<% if $multiple %> multiple="multiple"<% end_if %> />
				</label>
			<% else %>
				<input id="$id" name="file" class="$extraClass ss-uploadfield-fromcomputer-fileinput" data-config="$configString" type="hidden" />
			<% end_if %>
