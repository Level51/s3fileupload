/**
 * Try to reload img every 500ms if there was an error loading it
 * @param image
 */
function imageFailedLoading(image) {
  setTimeout(() => {
...
window.tmpl.cache['ss-s3uploadfield-downloadtemplate'] = tmpl(`${'{% for (var i=0, files=o.files, l=files.length, file=files[0]; i<l; file=files[++i]) { %}' +
		'<li class="ss-uploadfield-item template-download{% if (file.error) { %} ui-state-error{% } %}" data-fileid="{%=file.id%}">' +
			'{% if (file.thumbnail_url) { %}' +
				'<div class="ss-uploadfield-item-preview preview"><span>' +
					'<img src="{%=file.thumbnail_url%}" alt="" onerror="imageFailedLoading(this)" />' +
				'</span></div>' +
			'{% } %}' +
